...
Add a test (or untestable declaration) for rule **MA0209** — *Use in keyword for in parameter* —
from the `Meziantou.Analyzer` analyzer.

## Acceptance criteria

- [ ] Either a `[Fact]` with `[RuleDoc("MA0209", ...)]` exists in
      `tests/Opinionated.DotNet.CodingStandards.Tests/MeziantouAnalyzers/MeziantouAnalyzers3Should.cs`,
      or a class-level `[RuleDoc("MA0209", ..., Untestable = "...")]` exists in `UntestableRules.cs`
- [ ] `RuleDocCoverageShould` passes (no duplicate or missing `[RuleDoc]` entries)
- [ ] If testable: `dotnet test --no-build --filter "FullyQualifiedName~UseInKeywordForInParameter"` passes
...
public async Task UseInKeywordForInParameter()
{
    ...
    await project.AddFileAsync("Program.cs", """
        namespace test;
        // TODO: add code that triggers MA0209 — calling a method whose parameter is declared
        // `in` while passing the argument without the `in` keyword.
