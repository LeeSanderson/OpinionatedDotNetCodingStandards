...
Add a test (or untestable declaration) for rule **MA0207** — *[FixedAddressValueType] fields
must be static* — from the `Meziantou.Analyzer` analyzer.

## Acceptance criteria

- [ ] Either a `[Fact]` with `[RuleDoc("MA0207", ...)]` exists in
      `tests/Opinionated.DotNet.CodingStandards.Tests/MeziantouAnalyzers/MeziantouAnalyzers3Should.cs`,
      or a class-level `[RuleDoc("MA0207", ..., Untestable = "...")]` exists in `UntestableRules.cs`
- [ ] `RuleDocCoverageShould` passes (no duplicate or missing `[RuleDoc]` entries)
- [ ] If testable: `dotnet test --no-build --filter "FullyQualifiedName~RequireFixedAddressValueTypeFieldsToBeStatic"` passes
...
public async Task RequireFixedAddressValueTypeFieldsToBeStatic()
{
    ...
    await project.AddFileAsync("Program.cs", """
        namespace test;
        // TODO: add code that triggers MA0207 — a [System.Runtime.CompilerServices.FixedAddressValueType]
        //       attribute applied to a non-static field
