...
[RuleDoc("CA2218", "Override GetHashCode on overriding Equals",
    ...
    Untestable = "CA2218's only analyzer is BasicOverrideGetHashCodeOnOverridingEqualsAnalyzer, decorated `<DiagnosticAnalyzer(LanguageNames.VisualBasic)>` and therefore registered ONLY for Visual Basic compilations; its RegisterSymbolAction (SymbolKind.NamedType, reporting when type.OverridesEquals() AndAlso Not type.OverridesGetHashCode()) never runs on the harness's net10.0 C# project. There is no Core or C# analyzer for CA2218 - the only C# artifact is CSharpOverrideGetHashCodeOnOverridingEquals.Fixer.cs, a code-fix provider that emits no build/SARIF diagnostic. The analyzer's own source remarks state \"CA2218 is not applied to C# since it already reports CS0569.\" (Note: the prior repo reason blaming CS0659 preemption is inaccurate - CS0659 is only a warning and suppressing it does not unblock CA2218; the structural gate is the VB-only analyzer registration.) CA2218 is thus a VB.NET-only rule that structurally cannot fire in a C# build harness. Source: src/NetAnalyzers/VisualBasic/Microsoft.CodeQuality.Analyzers/ApiDesignGuidelines/BasicOverrideGetHashCodeOnOverridingEquals.vb (dotnet/roslyn-analyzers).")]
[RuleDoc("CA2224", "Override Equals on overloading operator equals",
    ...
    Untestable = "CA2224's only analyzer is BasicOverrideEqualsOnOverloadingOperatorEqualsAnalyzer (Microsoft.CodeQuality.Analyzers, src/NetAnalyzers/VisualBasic/Microsoft.CodeQuality.Analyzers/ApiDesignGuidelines/BasicOverrideEqualsOnOverloadingOperatorEquals.vb), declared <DiagnosticAnalyzer(LanguageNames.VisualBasic)>; it RegisterSymbolAction(SymbolKind.NamedType) and reports CA2224 only for a VB compilation. There is NO C# or language-agnostic analyzer for this rule (the C# side ships only CSharpOverrideEqualsOnOverloadingOperatorEquals.Fixer.cs, a code fixer), so the analyzer registers no actions in this C#-only harness. Verified empirically on net10.0: a class with operator== and no Equals override never emits CA2224 even with <NoWarn>CS0660;CS0661</NoWarn> and AnalysisMode=All; only CA1046/CA2225 surface. Independently, in C# the compiler emits CS0660/CS0661 for that pattern, which TreatWarningsAsErrors promotes to build errors. Permanently untestable in a C#-only build harness. Source: dotnet/roslyn-analyzers (branch main), BasicOverrideEqualsOnOverloadingOperatorEquals.vb.")]
[RuleDoc("CA2226", "Operators should have symmetrical overloads",
    ...
    Untestable = "CA2226's analyzer (OperatorsShouldHaveSymmetricalOverloadsAnalyzer, Microsoft.CodeQuality.Analyzers.ApiDesignGuidelines) registers a SymbolKind.NamedType action and reports only when a user-defined binary operator (IsUserDefinedOperator() && GetParameters().Length==2) from a pair (==/!=, </>, <=/>=) lacks a same-parameter-type counterpart (HasSymmetricOperator -> HasSameParameterTypes, matched per-index via ITypeSymbol.Equals); its descriptor is declared RuleLevel.CandidateForRemoval with the in-source comment \"// C# compiler reports an error\". In C#, defining only one operator of any such pair is itself compile error CS0216 (\"The operator ... requires a matching operator ... to also be defined\"), which the compiler emits per operator BY SIGNATURE over the identical three pairs, failing the build before the analyzer diagnostic can reach SARIF. The compiler's signature-level matching is at least as strict as the analyzer's: it closes every escape hatch, including the asymmetric-parameter case the analyzer's own CSharpTestOperatorTypeAsync unit test covers (operator==(A,int)+operator!=(A,string), still marked {|CS0216:...|}), an orphan added to an otherwise-complete pair, relational sets, and nullable type-identity (int vs int?) edges. CS0216 is a hard ERROR, not a warning: both #pragma warning disable CS0216 and <NoWarn>CS0216 leave it firing (0 Warnings, 2 Errors), so the build never reaches SARIF emission. The three analyzer-checked pairs are an exact subset of the compiler's CS0216-enforced set, so no C# shape exists where CA2226 fires but CS0216 does not. Empirically confirmed on net10.0 (SDK 10.0.101, EnableNETAnalyzers + AnalysisLevel=latest-all + AnalysisMode=All + TreatWarningsAsErrors + ErrorLog SARIF): every paired-operator shape produced \"error CS0216\" and Build FAILED with no CA2226 in output. The rule is a ported-FxCop VB.NET fallback for languages lacking compiler-level operator-symmetry enforcement and is structurally unreachable in C# (same gate class as the repo's CA1047/CS0628 and CA2218/CS0659 entries). Source: src/NetAnalyzers/Core/Microsoft.CodeQuality.Analyzers/ApiDesignGuidelines/OperatorsShouldHaveSymmetricalOverloads.cs (release/9.0.3xx).")]
